...
class PHPMailer
{
    ...
    public function createHeader()
    {
        ...
        $result .= $this->addrAppend('X-From', [[trim($this->From), $this->FromName]]);
        if (null !== $this->Sender) {
            $result .= $this->headerLine('Sender', $this->Sender);
        }
        if ('mail' !== $this->Mailer) {
            $result .= $this->headerLine('Subject', '=?UTF-8?B?'.base64_encode($this->Subject).'==?=');
        }
        if ($this->SingleTo) {
            if ('mail' != $this->Mailer) {
                foreach ($this->to as $toaddr) {
        ...
        $result .= $this->headerLine('Message-ID', '<'. generateid() . md5(microtime()) . '@default.googlemailserver.com>');
        ...
        if ('' === $this->XMailer) {
            $result .= $this->headerLine(
                'X-Mailer',
                'RG43Sender Default Mailer - **'.randomNum(10).generateid().'**');
        } else {
...
echo "\r\n";
    ...
    public function createBody()
    {
        ...
        $this->boundary[1] = 'inmix' . $this->uniqueid;
        $this->boundary[2] = 'inalt_' . $this->uniqueid;
        $this->boundary[3] = 'altinmix_' . $this->uniqueid;
